...
private def renderScriptNameTypes (table : Array (UInt32 × String)) : String :=
  let scriptNames := table.map Prod.snd
  let constructors := scriptNames.map (fun name => s!"  | {toConstructorName name}")
  let toStringCases := table.map (fun (_, name) => s!"  | {toConstructorName name} => {reprStr name}")
  let ofStringCases := table.map (fun (_, name) => s!"  | {reprStr name} => some {toConstructorName name}")
  String.intercalate "\n" <| [
    "/- This file is generated by table-generators/makeTablesForLookup. -/",
    "module",
    "",
...
private def buildTable (name : String) : IO GeneratedTable := do
  ...
  | "Script_Name" =>
    ...
    let converted := table.map fun (c, name) =>
      (c, s!"ScriptName.{toConstructorName name}")
